...
struct RoboPredictor::RoboMemory {
  ...
  std::uint32_t lastPlanet = 0;
  std::uint32_t lasttwoPlanet = 0;
  std::uint64_t lastthreePlanet = 0;
...
bool RoboPredictor::predictTimeOfDayOnNextPlanet(
  ...
  std::pair<std::uint64_t,std::uint32_t> lastPlanetPattern = std::make_pair((roboMemory_ptr->lastthreePlanet << 42) | (roboMemory_ptr->lasttwoPlanet << 21) |
                                    (roboMemory_ptr->lastPlanet), roboMemory_ptr->currentPlanet);

  std::pair<std::pair<std::uint64_t,std::uint32_t>, std::uint32_t> pair = std::make_pair(lastPlanetPattern, nextPlanetID);
...
void RoboPredictor::observeAndRecordTimeofdayOnNextPlanet(
  ...
  roboMemory_ptr->lastthreePlanet = roboMemory_ptr->lasttwoPlanet;
